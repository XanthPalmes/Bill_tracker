...
class TrackerUI {
  ...
  private updateTotals(): void {
    const totalEl = this._root.querySelector<HTMLElement>("[data-total]");
    if (totalEl) totalEl.textContent = this.money(this._manager.getTotal());
  }
  ...
  private renderGroups(): void {
    this._root.querySelectorAll<HTMLElement>("[data-group-card]").forEach((card) => {
      ...
      const group = this._manager.getGroups().find((g) => g.label === label);
      if (!group) return;
      ...
      sorted.forEach((item) => {
        ...
        const typeLabel = item.getBillTypeLabel();
        listItem.setAttribute("data-bill-type", typeLabel);
        listItem.innerHTML = `
          <div>
            <p class="bill-name">${item.name}</p>
            <p class="bill-note">${typeLabel}</p>
          </div>
          <span class="bill-value">${this.money(item.monthlyImpact())}</span>
          <button class="delete-button" type="button" data-delete-id="${item.id}" data-group="${group.label}">Delete</button>
        `;
